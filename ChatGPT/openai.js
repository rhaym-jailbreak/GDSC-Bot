    ...
    client.on("messageCreate", async message => {
        if (!message.author.bot && (message.channel.type == ChannelType.DM || targetWords.some(word => message.channel.name.includes(word)))) {
                    ...
                    stableDiffusion.generate(prompt, async (result) => {
                        ...
                        try {
                            ...
                            for (let i = 0; i < result.results.length; i++) {
                                ...
                                let attachment = new AttachmentBuilder(buffer, `result-${i}.jpg`)
                                attachments.push(attachment)
                            }
                            await message.reply({content: "Done", files: [attachments] })
                        } catch (e) {
                            console.error(e)
            ...
            try {
                ...
                }, 5000); // pause execution for 5000 milliseconds (5 seconds)
            } catch (e) {
                console.error(e)
            }
        }
